    ...
    function index()
     {
        ...
        else if($this->form_validation->run() == FALSE)
       {
        ...
        if (($this->input->get('user')) and ($this->input->get('key'))){
          $this->db->query("UPDATE eshop_users SET is_active=1 WHERE email='".$this->input->get('user')."' AND validity_key='".$this->input->get('key')."'");
          redirect('', 'refresh'); 
        }
        else {
          redirect('', 'refresh');
